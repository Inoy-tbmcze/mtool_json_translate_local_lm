...
def apply_block_patch_files(
    target_file: str | Path,
    search_file: str | Path,
    replace_file: str | Path,
    allow_multiple: bool = False,
    clean_tmp: bool = False,
) -> PatchResult:
    """Convenience wrapper that reads search and replace blocks from files."""
    s_path = Path(search_file)
    r_path = Path(replace_file)
    search_block = s_path.read_text(encoding="utf-8")
    replace_block = r_path.read_text(encoding="utf-8")
    res = apply_block_patch(target_file, search_block, replace_block, allow_multiple)
    if res.success and clean_tmp:
...
def main() -> int:
    ...
    parser.add_argument(
        "--search-file",
        help="Path to file containing search block (avoids shell quoting issues).",
    )
    parser.add_argument(
        "--replace-file",
        help="Path to file containing replacement block (avoids shell quoting issues).",
    )
    parser.add_argument(
        "--clean-tmp",
        action="store_true",
        help="Delete temporary search-file, replace-file, or payload-file on successful patch.",
    )
    ...
    if args.stdin:
        ...
        if args.search_file:
            try:
                search_val = Path(args.search_file).read_text(encoding="utf-8")
            except Exception as e:
                print(
                    json.dumps({"success": False, "error": f"Failed reading search file: {e}"}),
                    file=sys.stderr,
                )
                return 1
        ...
        if args.replace_file:
            try:
                replace_val = Path(args.replace_file).read_text(encoding="utf-8")
            except Exception as e:
                print(
                    json.dumps({"success": False, "error": f"Failed reading replace file: {e}"}),
                    file=sys.stderr,
                )
                return 1
        ...
        payload = {
            ...
            "search_block": search_val,
            "replace_block": replace_val,
    ...
    if res.success:
        if args.clean_tmp:
            if args.search_file:
                Path(args.search_file).unlink(missing_ok=True)
            if args.replace_file:
                Path(args.replace_file).unlink(missing_ok=True)
            if args.payload_file:
                Path(args.payload_file).unlink(missing_ok=True)
        print(out_json)
        return 0
    else:
