...
------------------------------------------------------------------
Validates Japanese -> English translations. Items returning 0 (unrelated or 
hallucinated translations) are saved to a re-translation file in the format 
...
def call_batch_validation(batch: List[Tuple[int, str, str]], config: Dict[str, Any]) -> Dict[str, bool]:
    ...
    combined_prompt = (
        "You are a translation quality auditor. Check if the English (EN) text is a valid, "
        "plausible translation or equivalent representation of the Japanese (JP) source text.\n\n"
        "Rules:\n"
        "- Return 1 if the EN text accurately matches or contextually represents the JP source.\n"
        "- Return 0 if the EN text is unrelated, completely incorrect, hallucinated, or un-translated garbage.\n\n"
...
def process_validation(config_file: str = "config_validate.json"):
    ...
    print(f"--- Validating {len(items)} translated lines across {len(batches)} batches ---")
                ...
                with lock:
                    for idx, jp, en in batch:
                        ...
                        if is_valid:
                            ...
                            # Reset to source key for round 2 translation
                            retranslate_data[jp] = jp

        print(f"Wave {wave_idx}/{len(waves)} complete.")

    # Save validated (passed) lines
    with open(valid_path, "w", encoding="utf-8") as f:
        json.dump(validated_data, f, ensure_ascii=False, indent=2)

    # Save failed lines ready for second pass
    ...
    print(f"Passed (Valid):        {len(validated_data)} -> {valid_path.name}")
    print(f"Failed (Retranslate):   {len(retranslate_data)} -> {retranslate_path.name}")
